...
- (void)panGestureRecognized:(id)sender {
    ...
    if ([(UIPanGestureRecognizer*)sender state] == UIGestureRecognizerStateEnded) {
        if(scrollView.center.y > viewHalfHeight+40 || scrollView.center.y < viewHalfHeight-40 ||
                scrollView.center.x > viewHalfWidth+40 || scrollView.center.x < viewHalfWidth-40) // Automatic Dismiss View
        {
            ...
            CGFloat finalX = scrollView.center.x, finalY = scrollView.center.y;

            CGFloat windowsHeight = [_applicationWindow frame].size.height;
            CGFloat windowsWidth = [_applicationWindow frame].size.width;

            if (abs(newY) > abs(newX)) {
                if (scrollView.center.y > viewHalfHeight + 30) // swipe down
                    finalY = windowsHeight * 2;
                else // swipe up
                    finalY = -viewHalfHeight;
            } else {
                if (scrollView.center.x > viewHalfWidth + 30) // swipe right
                    finalX = windowsWidth * 2;
                else // swipe left
                    finalX = -viewHalfWidth;
            }
